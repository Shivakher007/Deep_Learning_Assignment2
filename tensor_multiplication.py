...
def calculate_activation(sum_total):
    ...
    """
    # Check if each element of sum_total is zero
    zero_mask = torch.eq(sum_total, 0)

    # Initialize the result tensor with zeros
    res = torch.zeros_like(sum_total)

    # For elements where sum_total is zero, set res to 0.0
    res[zero_mask] = 0.0

    # For other elements, use PyTorch heaviside function
    res[~zero_mask] = torch.heaviside(sum_total[~zero_mask], torch.tensor(0.5))
